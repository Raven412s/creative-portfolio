  ...
  return (
    <div className="flex gap-2 mt-5 flex-wrap">
      {tags.map((t, i) => (
        <span
          key={t}
          className={`text-[10px] font-mono px-3 py-1 rounded-full border transition-all ${i === highlight
...
/* ─────────────────────────────────────────
   Panel 2 — Blog
───────────────────────────────────────── */
export function BlogPanel({ tags }: { tags: string[] }) {
  return (
...
/* ─────────────────────────────────────────
   Panel 3 — Auth
───────────────────────────────────────── */
export function AuthPanel({ tags }: { tags: string[] }) {
  return (
    <div className="panel-inner flex flex-col justify-center h-full px-6 sm:px-12 lg:px-16 w-full">
...
/* ─────────────────────────────────────────
   Panel 4 — Starter Kit
───────────────────────────────────────── */
export function StarterPanel({ tags }: { tags: string[] }) {
  const lines = [
...
───────────────────────────────────────── */
export function LocalePanel({ tags }: { tags: string[] }) {
...
───────────────────────────────────────── */
export function CompLibPanel({ tags }: { tags: string[] }) {
  const components = [
...
/* ─────────────────────────────────────────
   Panel renderer
───────────────────────────────────────── */
export function renderPanel(p: (typeof panels)[0]) {
  switch (p.type) {
...
export function DisplaySection() {
  ...
  useEffect(() => {
    ...
    // ── Scope all GSAP work to this element so ctx.revert() only
    //    kills DisplaySection's own triggers, never AboutSection's pin ──
    const ctx = gsap.context(() => {

      if (isMobile) {
        // Mobile: no pin, no horizontal scroll.
        // Just handle the dark→light bg via inline style since
        // the section is auto-height and not pinned.
      ...
      gsap.to(track, {
        // Fix 3: arrow function so x is recalculated on every ST refresh,
        // not captured once at setup time (stale after fonts/images load).
        ...
        scrollTrigger: {
          ...
          scrub: 0.1,
          start: "top top",
          // Fix 3: also lazy — recalculated on refresh.
          end: () => `+=${track.scrollWidth - window.innerWidth}`,
          anticipatePin: 1,
          // markers: true,

          // Fix 3: tells GSAP to re-run both callbacks above whenever
          // ScrollTrigger.refresh() fires (after About's pin settles,
          // after fonts load, after resize).
          invalidateOnRefresh: true,
          onUpdate: (self) => {
            if (bar) bar.style.width = `${self.progress * 100}%`;

            // Fix 2: drive the dark→light bg from horizontal scroll
            // progress instead of a separate scrollTrigger with
            // "top 50%" — that separate trigger fired at the wrong
            // pixel position because About's pin shifted the page.
            // Only blend during the first 15% of scroll (intro panel).
            const t = Math.min(self.progress / 0.15, 1);
            if (t < 1) {
              // lerp #0a0a0a → #fefefe
              const v = Math.round(t * (0xfe - 0x0a) + 0x0a);
              const hex = v.toString(16).padStart(2, "0");
              section.style.backgroundColor = `#${hex}${hex}${hex}`;
            } else {
              section.style.backgroundColor = "#fefefe";
            }
          },
      ...
      // Return inner cleanup for the listeners (ctx.revert handles GSAP)
      return () => {
    ...
    }, section); // ← scope to sectionRef, not the whole document

    // Fix 1: ctx.revert() only kills triggers created inside this context.
    // AboutSection's pin is untouched.
    return () => ctx.revert();
  }, []);
          ...
          {panels.map((p, i) => {
            ...
            return (
              <div
                ...
                className={`panel-card shrink-0 flex items-center relative border-r border-neutral-100 last:border-r-0 first:border-r-0 ${isIntro ? "intro-card intro-panel-bg" : "blueprint-bg"
                  }`}
                style={{
