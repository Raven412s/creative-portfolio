...
export interface CursorElementOptions {
   ...
   */
  icon?: ReactNode;
  /** Image URL shown as a preview (circle or window) */
  mediaSrc?: string;
  /** Shape of the media preview: "circle" (default) or "window" (16:9 aspect-ratio rectangle) */
  mediaShape?: "circle" | "window";
  /** Custom text/icon color. Overrides default contrast colors */
  textColor?: string;
  /** Custom circle background color. Overrides default background */
 ...
 *
 * @example  media with window shape (16:9 rectangle)
 * const h = useCursorElement({ state: 'media', mediaSrc: '/thumb.jpg', mediaShape: 'window' })
 *
 * @example  custom text color
 * const h = useCursorElement({ state: 'text', text: 'example', textColor: '#ffffff' })
 *
 * @example  custom background color
 * const h = useCursorElement({ state: 'icon', icon: '↗', backgroundColor: '#ff5733' })
 *
 * @example  both colors together
